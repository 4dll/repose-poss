...
import { TABLE_COUNT, cancelOpenOrder, createOpenTableOrder, createTakeawayOrder, getOpenTableOrder, getOrderWithLines, payOrder, updateOpenOrder, } from "./orders.js";
initDb();
const app = express();
app.use(cors());
...
app.get("/api/staff", (_req, res) => {
    res.json(db.prepare("SELECT id, name, username FROM staff ORDER BY id").all());
});
app.get("/api/shifts/active", (_req, res) => {
    const shifts = db
        .prepare(`SELECT s.*, st.name as staff_name, st.username as staff_username
       FROM shifts s
...
app.post("/api/shifts/open", (req, res) => {
    ...
    db.prepare("INSERT INTO shifts (staff_id, started_at) VALUES (?, datetime('now'))").run(staff.id);
    const shiftId = db.prepare("SELECT last_insert_rowid() as id").get();
    const shift = db
        .prepare(`SELECT s.*, st.name as staff_name FROM shifts s
...
function getShiftReport(shiftId) {
    const shift = db
       ...
       JOIN staff st ON st.id = s.staff_id WHERE s.id = ?`)
        .get(shiftId);
    const lines = db
        .prepare(`SELECT ol.*, o.created_at as order_time, o.discount_amount, o.discount_type, o.discount_value
       FROM order_lines ol
    ...
    const discountTotal = db
        .prepare(`SELECT COALESCE(SUM(discount_amount), 0) as total FROM orders
...
app.post("/api/categories", (req, res) => {
    ...
    const maxOrder = db.prepare("SELECT COALESCE(MAX(sort_order), 0) as m FROM categories").get();
    try {
        db.prepare("INSERT INTO categories (name, sort_order) VALUES (?, ?)").run(name.trim(), maxOrder.m + 1);
    }
    catch {
        return res.status(400).json({ error: "Category already exists" });
    }
    const newId = db.prepare("SELECT last_insert_rowid() as id").get();
...
app.post("/api/menu", (req, res) => {
    ...
    db.prepare(`INSERT INTO menu_items (name, price, stock_qty, low_stock_threshold, category_id)
     VALUES (?, ?, ?, ?, ?)`).run(name, price, stockQty ?? 0, lowStockThreshold ?? 5, categoryId);
    const newId = db.prepare("SELECT last_insert_rowid() as id").get();
    res.json(db.prepare(`${menuSelect()} WHERE m.id = ?`).get(newId.id));
});
app.patch("/api/menu/:id", (req, res) => {
    ...
    db.prepare(`UPDATE menu_items SET
     ...
     WHERE id = ?`).run(name ?? null, price ?? null, stockQty ?? null, lowStockThreshold ?? null, active ?? null, req.body.categoryId ?? null, id);
...
app.get("/api/tables", (_req, res) => {
    ...
    for (let n = 1; n <= TABLE_COUNT; n++) {
        const open = getOpenTableOrder(n);
        if (open) {
            const itemCount = db
...
app.post("/api/tables/:number/open", (req, res) => {
    ...
    if (!shiftId)
        return res.status(400).json({ error: "Shift required" });
    ...
    if (!shift)
        return res.status(400).json({ error: "Shift not open" });
    try {
...
app.post("/api/orders", (req, res) => {
    const { shiftId, lines, discountType, discountValue, cashAmount, visaAmount, paymentMethod } = req.body;
    ...
    try {
        ...
        const result = createTakeawayOrder(shiftId, lines, { cashAmount: cash, visaAmount: visa }, discountType, discountValue);
        db.exec("COMMIT");
        res.json(result);
    }
    catch (e) {
...
app.get("/api/stock", (_req, res) => {
    res.json(db
        .prepare(`SELECT m.*, c.name as category_name,
...
app.post("/api/stock/:id/adjust", (req, res) => {
    ...
    try {
        ...
        db.prepare("INSERT INTO stock_adjustments (menu_item_id, qty_change, reason) VALUES (?, ?, ?)").run(id, qtyChange, reason || null);
        db.exec("COMMIT");
    }
    catch (e) {
        db.exec("ROLLBACK");
...
function getPeriodReport(from, to) {
    const totals = db
        .prepare(`SELECT
         COALESCE(SUM(o.cash_amount), 0) as cash_total,
         COALESCE(SUM(o.visa_amount), 0) as visa_total,
    ...
    const discounts = db
       ...
       WHERE date(created_at) >= date(?) AND date(created_at) <= date(?)
        ...
        .get(from, to);
    const byDay = db
        .prepare(`SELECT date(o.created_at) as day,
...
app.get("/api/reports/items", (req, res) => {
    ...
    const totals = db
        .prepare(`SELECT ol.item_name, SUM(ol.qty) as qty_sold, SUM(ol.line_total) as revenue
       FROM order_lines ol
       JOIN orders o ON o.id = ol.order_id
       WHERE date(o.created_at) >= date(?) AND date(o.created_at) <= date(?)
         AND o.status = 'paid'
