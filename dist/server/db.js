...
export function initDb() {
  ...
  `);
    ...
    db.exec(`UPDATE orders SET status = 'paid', service_type = 'takeaway'
     WHERE status IS NULL OR status = ''`);
    db.exec(`UPDATE orders SET cash_amount = total, visa_amount = 0
    ...
    db.exec(`UPDATE orders SET cash_amount = 0, visa_amount = total
     WHERE status = 'paid' AND payment_method = 'visa' AND visa_amount = 0`);
    db.exec(`CREATE UNIQUE INDEX IF NOT EXISTS idx_open_dine_table
...
function ensureDefaultCategories() {
    ...
    const hot = db.prepare("SELECT id FROM categories WHERE name = 'Hot Drinks'").get();
    const cold = db.prepare("SELECT id FROM categories WHERE name = 'Cold Drinks'").get();
    const food = db.prepare("SELECT id FROM categories WHERE name = 'Food'").get();
    ...
    for (const n of coldNames)
        update.run(cold.id, n);
    for (const n of foodNames)
        update.run(food.id, n);
