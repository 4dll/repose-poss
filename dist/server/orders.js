...
export function calcTotals(lines, discountType, discountValue) {
...
export function getOpenTableOrder(tableNumber) {
    return db
        .prepare(`SELECT * FROM orders
       WHERE status = 'open' AND service_type = 'dine_in' AND table_number = ?`)
        .get(tableNumber);
}
export function getOrderWithLines(orderId) {
...
function validateStock(lines) {
    for (const line of lines) {
        const item = db.prepare("SELECT * FROM menu_items WHERE id = ? AND active = 1").get(line.menuItemId);
        if (!item)
...
function restoreStockFromOrder(orderId) {
    const oldLines = db
        .prepare("SELECT menu_item_id, qty FROM order_lines WHERE order_id = ?")
        .all(orderId);
    for (const line of oldLines) {
        db.prepare("UPDATE menu_items SET stock_qty = stock_qty + ? WHERE id = ?").run(line.qty, line.menu_item_id);
    }
}
export function replaceOrderLines(orderId, lines, paymentMethod) {
    validateStock(lines);
    restoreStockFromOrder(orderId);
    db.prepare("DELETE FROM order_lines WHERE order_id = ?").run(orderId);
    const insertLine = db.prepare(`INSERT INTO order_lines (order_id, menu_item_id, item_name, qty, unit_price, line_total, payment_method)
     VALUES (?, ?, ?, ?, ?, ?, ?)`);
    for (const line of lines) {
        const item = db.prepare("SELECT name FROM menu_items WHERE id = ?").get(line.menuItemId);
        const lineTotal = line.qty * line.unitPrice;
        insertLine.run(orderId, line.menuItemId, item.name, line.qty, line.unitPrice, lineTotal, paymentMethod);
        db.prepare("UPDATE menu_items SET stock_qty = stock_qty - ? WHERE id = ?").run(line.qty, line.menuItemId);
    }
}
export function createOpenTableOrder(shiftId, tableNumber) {
    ...
    ) VALUES (?, 'dine_in', ?, 'open', NULL, 0, 0, 0, 0)`).run(shiftId, tableNumber);
    return db.prepare("SELECT last_insert_rowid() as id").get().id;
}
export function updateOpenOrder(orderId, lines, discountType, discountValue) {
    const order = db.prepare("SELECT * FROM orders WHERE id = ?").get(orderId);
    if (!order)
        throw new Error("Order not found");
    if (order.status !== "open")
        throw new Error("Order is already paid");
    const { subtotal, discountAmount, total, discountValue: dVal } = calcTotals(lines, discountType, discountValue);
    replaceOrderLines(orderId, lines, "cash");
    db.prepare(`UPDATE orders SET
      discount_type = ?, discount_value = ?, subtotal = ?, discount_amount = ?, total = ?
     WHERE id = ?`).run(discountType || null, dVal, subtotal, discountAmount, total, orderId);
    return getOrderWithLines(orderId);
}
export function validatePayment(total, cashAmount, visaAmount) {
...
function paymentLabel(cashAmount, visaAmount) {
    if (cashAmount > 0 && visaAmount > 0)
        return "split";
    if (visaAmount > 0)
        return "visa";
    return "cash";
...
function applySplitToLines(orderId, cashAmount, visaAmount) {
    const lines = db
...
export function payOrder(orderId, payment) {
    const order = db.prepare("SELECT * FROM orders WHERE id = ?").get(orderId);
    if (!order)
        throw new Error("Order not found");
    if (order.status !== "open")
    ...
    const lineCount = db
        ...
        .get(orderId);
    if (lineCount.c === 0)
        throw new Error("Add items before payment");
    ...
    db.prepare(`UPDATE orders SET status = 'paid', payment_method = ?, cash_amount = ?, visa_amount = ?,
      updated_at = datetime('now') WHERE id = ?`).run(label, cashAmount, visaAmount, orderId);
    return getOrderWithLines(orderId);
}
export function createTakeawayOrder(shiftId, lines, payment, discountType, discountValue) {
    const { subtotal, discountAmount, total, discountValue: dVal } = calcTotals(lines, discountType, discountValue);
    ...
    db.prepare(`INSERT INTO orders (
      ...
      discount_type, discount_value, subtotal, discount_amount, total
    ) VALUES (?, 'takeaway', NULL, 'paid', ?, ?, ?, ?, ?, ?, ?, ?)`).run(shiftId, label, cashAmount, visaAmount, discountType || null, dVal, subtotal, discountAmount, total);
...
export function cancelOpenOrder(orderId) {
    const order = db.prepare("SELECT * FROM orders WHERE id = ?").get(orderId);
