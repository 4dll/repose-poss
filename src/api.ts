...
export const api = {
  ...
  stock: async () => { const data = store(); return data.menu.map((item) => menuWithCategory(data, item)); },
  ...
  deleteMenuItem: async (id: number) => { const data = store(); if (data.lines.some((line) => line.menu_item_id === id)) throw new Error("This item appears in past orders and cannot be deleted"); data.menu = data.menu.filter((item) => item.id !== id); save(data); return { ok: true }; },
